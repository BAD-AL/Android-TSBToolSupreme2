...
public class SNES_TecmoTool implements ITecmoTool
{
    ...
    //public SNES_TecmoTool(String fileName) { Init(fileName); }
    ...
    public boolean Init(byte[] romBytes) {
    ...
    public void SaveRom(OutputStream outputStream) throws IOException {
        if (outputStream != null)
        {
            long len = outputRom.length;
            outputStream.write(outputRom, 0, (int) len);
            outputStream.close();
        }
        else {
            errors.add("ERROR! You passed a null filename");
